...
def adaptive_arxiv_fetch_limits(days: int) -> tuple[int, int]:
    """Return (batch_size, fetch_budget) for arXiv multi-day windows.

    Wider windows often need pagination because a single 3000-result page can miss
    older in-range papers when the queried categories are busy. We scale the total
    fetch budget with the window size, but still stop early once the oldest fetched
    page has crossed the requested start date.
    """
...
def fetch_arxiv_papers(start_date=None, end_date=None, days: int = 1) -> list[dict]:
    ...
    for start in range(0, fetch_budget, batch_size):
        ...
        url = (
            ...
            f"&start={start}&max_results={current_max}"
        )

        print(
            f"    arXiv page start={start}, max_results={current_max}...",
            file=sys.stderr,
        )
    ...
    print(
        f"  arXiv: {len(scored)} papers after scoring (from {len(papers)} parsed across {fetched_entries} fetched entries, {filtered_by_date} filtered by date)",
        file=sys.stderr,
    )
    return scored
