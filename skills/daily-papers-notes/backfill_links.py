#!/usr/bin/env python3
"""
backfill_links.py - Backfill paper note links into a recommendation file.

This script is part of daily-papers-notes (Step 3).

Usage:
    python3 backfill_links.py --recommendation YYYY-MM-DD-paper-recommendations.md
    python3 backfill_links.py --recommendation YYYY-MM-DD-paper-recommendations.md --notes-dir PaperNotes

The script:
1. Scans the notes directory for existing paper notes
2. Matches papers in the recommendation file with existing notes
3. Inserts note links after the "Source" line for each matched paper
4. Updates the "Triage Table" section to use the correct wikilink names
"""

import argparse
import re
import sys
from pathlib import Path

_SHARED_DIR = Path(__file__).resolve().parent.parent / "_shared"
if str(_SHARED_DIR) not in sys.path:
    sys.path.insert(0, str(_SHARED_DIR))

from user_config import paper_notes_dir, paths_config

NOTES_DIR = paper_notes_dir()
CONCEPTS_DIR_NAME = paths_config()["concepts_folder"]


def _extract_frontmatter_method_name(md_file: Path) -> str:
    try:
        content = md_file.read_text(encoding="utf-8")
    except OSError:
        return ""
    match = re.search(r'^method_name:\s*["\']?(.+?)["\']?\s*$', content, re.MULTILINE)
    return match.group(1).strip() if match else ""


def scan_notes(notes_dir: Path | None = None) -> dict:
    """Scan notes directory and build an index of {method_name: note_path}."""
    notes_index = {}
    notes_root = notes_dir or NOTES_DIR

    if not notes_root.exists():
        return notes_index

    for md_file in notes_root.rglob("*.md"):
        if CONCEPTS_DIR_NAME in md_file.parts:
            continue
        if md_file.name == "_index.md":
            continue
        if md_file.parent == notes_root and md_file.stem == notes_root.name:
            continue
        if md_file.parent.name == md_file.stem:
            continue

        names = {md_file.stem}
        method_name = _extract_frontmatter_method_name(md_file)
        if method_name:
            names.add(method_name)

        for candidate in names:
            notes_index[candidate.lower()] = {
                "name": md_file.stem,
                "path": md_file.relative_to(notes_root.parent),
            }

    return notes_index


def extract_method_name_from_title(title: str) -> str:
    """Extract method name from paper title.

    Examples:
        "NavThinker: Action-Conditioned..." -> "NavThinker"
        "HapticVLA: Contact-Rich..." -> "HapticVLA"
        "ForceVLA2: Unleashing..." -> "ForceVLA2"
    """
    if ":" in title:
        method_name = title.split(":")[0].strip()
        method_name = re.sub(r"^\d+\.\s*", "", method_name)
        return method_name
    return title.split()[0] if title else ""


def match_papers_with_notes(content: str, notes_index: dict) -> list:
    """Match papers in a recommendation file with existing notes.

    Returns dictionaries with paper_title, method_name, note_name, section_start, and source_line_end.
    """
    matches = []

    for m in re.finditer(r"^### \d+\. (.+)$", content, re.MULTILINE):
        paper_title = m.group(1).strip()
        section_start = m.start()

        next_section = re.search(r"^### (?:\d+\.|\w)", content[section_start + 1 :], re.MULTILINE)
        section_end = section_start + next_section.start() if next_section else len(content)

        section_content = content[section_start:section_end]
        method_name = extract_method_name_from_title(paper_title)

        source_match = re.search(r"^- \*\*Source\*\*:.*$", section_content, re.MULTILINE)
        if not source_match:
            continue

        source_line_end = source_match.end()

        if re.search(r"- 📒 \*\*(?:Existing Note|Note)\*\*:", section_content):
            continue

        method_lower = method_name.lower()
        if method_lower in notes_index:
            matches.append(
                {
                    "paper_title": paper_title,
                    "method_name": method_name,
                    "note_name": notes_index[method_lower]["name"],
                    "section_start": section_start,
                    "source_line_end": section_start + source_line_end,
                }
            )

    return matches


def backfill_links(recommendation_path: Path, notes_index: dict) -> int:
    """Backfill note links into a recommendation file."""
    with open(recommendation_path, "r", encoding="utf-8") as f:
        content = f.read()

    matches = match_papers_with_notes(content, notes_index)

    if not matches:
        print("No papers matched with existing notes")
        return 0

    for match in reversed(matches):
        insert_text = f'\n- 📒 **Note**: [[{match["note_name"]}]]'
        content = content[: match["source_line_end"]] + insert_text + content[match["source_line_end"] :]

    with open(recommendation_path, "w", encoding="utf-8") as f:
        f.write(content)

    update_triage_table(recommendation_path, notes_index, matches)

    return len(matches)


def update_triage_table(recommendation_path: Path, notes_index: dict, matches: list):
    """Update the Triage Table section to use correct wikilink names."""
    with open(recommendation_path, "r", encoding="utf-8") as f:
        content = f.read()

    table_match = re.search(r"^## Triage Table$.+?(?=^##|\Z)", content, re.MULTILINE | re.DOTALL)
    if not table_match:
        return

    table_start = table_match.start()
    table_end = table_match.end()
    table_content = content[table_start:table_end]

    for match in matches:
        if match["method_name"].lower() != match["note_name"].lower():
            table_content = re.sub(
                rf"\[\[{re.escape(match['method_name'])}\]\]",
                f"[[{match['note_name']}]]",
                table_content,
                flags=re.IGNORECASE,
            )

    content = content[:table_start] + table_content + content[table_end:]

    with open(recommendation_path, "w", encoding="utf-8") as f:
        f.write(content)


def main():
    parser = argparse.ArgumentParser(description="Backfill paper note links")
    parser.add_argument("--recommendation", required=True, help="Path to recommendation file")
    parser.add_argument("--notes-dir", help="Path to notes directory (default: from config)")

    args = parser.parse_args()

    recommendation_path = Path(args.recommendation)
    if not recommendation_path.exists():
        print(f"Error: Recommendation file not found: {recommendation_path}", file=sys.stderr)
        sys.exit(1)

    notes_dir = Path(args.notes_dir).expanduser() if args.notes_dir else NOTES_DIR

    notes_index = scan_notes(notes_dir)
    print(f"Found {len(notes_index)} paper notes")

    count = backfill_links(recommendation_path, notes_index)
    print(f"Added {count} note links to recommendation file")


if __name__ == "__main__":
    main()
