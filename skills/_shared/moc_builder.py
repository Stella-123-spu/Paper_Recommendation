...
def build_tree_mocs(
    *,
    ...
    for directory in directories:
        ...
        moc_path = _moc_path(directory)
        _remove_legacy_moc(directory)
...
def _note_files(directory: Path) -> list[Path]:
    return sorted(
        (
            ...
            and path.name != MOC_FILENAME
            and not _is_legacy_moc(path)
        ),
        key=lambda path: path.name,
    )
...
def _moc_path(directory: Path) -> Path:
    return directory / MOC_FILENAME


def _legacy_moc_path(directory: Path) -> Path:
    return directory / f"{directory.name}.md"


def _is_legacy_moc(path: Path) -> bool:
    return path == _legacy_moc_path(path.parent)


def _remove_legacy_moc(directory: Path) -> None:
    legacy_path = _legacy_moc_path(directory)
    ...
    try:
        ...
        return
    if "generated_by: dailypaper-skills" in content:
        legacy_path.unlink()
